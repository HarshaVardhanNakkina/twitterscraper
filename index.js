...
async function scrapeUser(user) {
	...
	const linuxPath = `/home/kennys/Pictures/twitterscraper/${account.name}/`;
	const windowsPath = `C:/Users/lenovo/Pictures/twitterscraper/${account.name}/`;
	// console.log(picsLinks);
	if (picsLinks.length === 0)
		console.log(`no new pics found for ${account.name}\n`);
	else getPicsAndStore(picsLinks, windowsPath);
}
